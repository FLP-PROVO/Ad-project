...
def test_expired_token_is_rejected(client, db_session: Session, tmp_path, monkeypatch) -> None:
    ...
    viewer, token = _create_viewer_with_token(db_session, email="viewer-expiry@example.com")
    media_file = storage.base_dir / "ads" / "sample.mp4"
    media_file.parent.mkdir(parents=True, exist_ok=True)
    media_file.write_bytes(SAMPLE_MP4_BYTES)

    signed_url = storage.generate_signed_url("/media/ads/sample.mp4", viewer.id, expires_seconds=1)
    time.sleep(2)
    parsed = urlparse(signed_url)

    response = client.get(parsed.path + "?" + parsed.query, headers={"Authorization": f"Bearer {token}"})
